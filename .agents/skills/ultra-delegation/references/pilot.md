...
`input_tokens` must include the actual serialized worker packet, host instructions and tool overhead; `output_tokens` reserves worker output. They are caller-supplied observations, not estimates derived by this tool from the short routing summary. Unknown worker limits cannot pass capacity checks. `available` means the exact native model and effort combination was discovered, not merely that the provider exists. Discovery and recheck are cooperative host protocols, not a sandbox or proof against falsified packets.
