...
This legacy helper section is historical. It does not describe the active pilot workflow. Prepare bounded summaries and exact discovered profiles with sanitized evidence. Routing-summary permission does not authorize artifact sharing. Preview a new packet shape with `--dry-run`; active native dispatch remains subject to the pilot's per-attempt recheck and coordinator acceptance.
