...
Pilot policy and evidence are separate from the legacy helper below. In the pilot, prompt wording versions are audit metadata; a material prompt contract or tool-policy change separates capability evidence. For a bounded read-only Codex trial, use [pilot-codex.md](references/pilot-codex.md) and its explicit discovery packet builder. Use effective routes, perform `recheck`, and execute through the native host. Trials are proposals until the coordinator has an authorized isolated execution plan. Keep comparisons scoped, and do not turn synthetic demos or Jev shadow scores into observed worker quality. Use [pilot-demands.md](references/pilot-demands.md) to translate demand signals into candidate evidence and independent review; they are not global complexity vetoes.
