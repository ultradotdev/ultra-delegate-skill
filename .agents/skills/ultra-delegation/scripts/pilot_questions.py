...
ROUTING_VERSION = "pilot-routing-v10"
SECURITY_VERSION = "pilot-security-v2"
MAX_CANDIDATES = 12
MAX_COHORTS = 2
MAX_REQUIREMENTS = 24
MAX_SECURITY_REQUIREMENTS = 12
SHARED_CAPABILITY_REF = "Shared prefix: `candidate_capability_prefix`.\n"
ROUTING_STATE_NOTE = ("Identical complete leading lines of candidate capability descriptions may be stored once "
                      "in state.candidate_capability_prefix, with an explicit reference in each description. "
                      "The prefix is part of every candidate description; all remaining text, evidence, and "
                      "questions are retained. This is lossless factoring, not truncation. Oversized payloads still fail.")
...
def route_payload(task, candidates, model):
    ...
    state = {"task": deepcopy(task), "candidates": deepcopy(candidates)}
    if len(candidates) > 1:
        common = os.path.commonprefix([c["capability_description"] for c in candidates])
        prefix = common[:common.rfind("\n") + 1]
        # Only share whole identical lines when doing so reduces wire size.
        if len(prefix) >= 128 and (len(candidates)-1)*len(prefix) > len(candidates)*len(SHARED_CAPABILITY_REF)+64:
            state["candidate_capability_prefix"] = prefix
            for candidate in state["candidates"]:
                candidate["capability_description"] = SHARED_CAPABILITY_REF + candidate["capability_description"][len(prefix):]
    return {"model": model, "state": state, "questions": q}
...
def manifest():
    ...
    shared = "Shared research provenance and limitations; effort-specific success is unknown and local reviewed outcomes remain separate. " * 2 + "\n"
    shared_candidates = [{**deepcopy(candidates[0]), "id": "shared-"+str(i), "capability_description": shared+text}
                         for i, text in enumerate(("Bounded code repair.", "Focused test writing."))]
    ...
    return {"schema": "ultra-delegation-pilot-questions-v1", "routing_version": ROUTING_VERSION,
            ...
            "routing_state_note": ROUTING_STATE_NOTE, "proposed_thresholds": PROPOSED_THRESHOLDS, "routing_registry": list(ROUTING_REGISTRY),
            ...
            "routing_shared_example": route_payload(task, shared_candidates, "jev-1.13.0"),
